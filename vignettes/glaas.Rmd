...
The full dataset is available directly after loading the `glaas`. No need to call `data("glaas")`! This package uses [lazy loading](https://journal.r-project.org/articles/RN-2004-009/RN-2004-009.pdf), which means the dataset is only loaded into memory when you actually use it. This keeps the package lightweight.

## What is GLAAS?

The `glaas` dataset contains data from the UN-Water Global Analysis and Assessment of Sanitation and Drinking-water survey, in short GLAAS. 
...
The dataset includes `r nrow(glaas)` rows and `r ncol(glaas)` variables covering the survey cycles from `r min(glaas$time_period)`-`r max(glaas$time_period)`.
...
First and foremost, the main goal of `glaas` is to make the data accessible for WASH practitioners, researchers, and other end users. With this package you can reproduce analyses and visualizations without the need for extensive data wrangling. To illustrate this, we have reproduced two graphics from the most recent report, ["State of systems for drinking-water, sanitation and hygiene - Global update 2025"](https://iris.who.int/server/api/core/bitstreams/83d75861-6c96-4083-b163-8accb9c711cc/content), using `glaas`.
...
```{r, fig.width = 7, fig.height = 5, fig.alt = "Grouped bar chart showing percentage of countries with measures fully in place for worker rights in drinking-water and sanitation sectors"}
...
```{r, fig.width = 8, fig.height = 5, fig.alt = "Horizontal stacked bar chart showing existence and status of WASH financing plans across service areas"}
...
Another example of how you can use data from `glaas` is mapping. The `country_code` variable (ISO 3-letter codes) makes it easy to join GLAAS data with geographic boundaries. You can find two example maps below. 
...
```{r, fig.width = 9, fig.height = 5, fig.alt = "World map showing GLAAS survey participation from 2013-2024 with colors indicating number of cycles each country participated in"}
# Get world map
...
```{r, fig.width = 9, fig.height = 5, fig.alt = "World map showing sufficiency of funding to reach national drinking-water targets based on 2024 GLAAS survey"}
...
Finally, `glaas` makes it easy to look at relationships between variables. For example, the heatmap below explores the relationship between financing plan status and funding sufficiency for urban drinking-water services. Do countries with more developed financing plans tend to have better funding coverage?

```{r, fig.width = 8, fig.height = 5, fig.alt = "Heatmap showing relationship between financing plan status and funding sufficiency for urban drinking-water services"}
